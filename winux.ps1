...
$chocoApps = @("nvidia-display-driver", "vnc-viewer", "unity", "cmdermini", "anydesk", "pushbullet", "obs-studio", "docker", "mingw", "tixati", "7zip", "bulk-crap-uninstaller", "caffeine", "clipgrab", "discord", "ditto", "slack", "google-cast-chrome", "lightshot", "skype", "vlc", "WhatsApp", "GoogleChrome", "VisualStudioCode")
$chocoApps | ForEach-Object { choco install -y --allow-empty-checksums $_ }
...
New-ItemProperty -path "hkcu:\SOFTWARE\Microsoft\Windows Search" -name "AllowCortana" -PropertyType DWORD -value 0 -Force
New-ItemProperty -path "hklm:\SOFTWARE\Microsoft\Windows Search" -name "AllowCortana" -PropertyType DWORD -value 0 -Force
New-ItemProperty -path "hklm:\SOFTWARE\Microsoft\Windows\Windows Search" -name "AllowCortana" -PropertyType DWORD -value 0 -Force
