...
$DISPLAY = (Get-NetAdapter "vEthernet (DockerNAT)" | Get-NetIPAddress -AddressFamily "IPv4").IPAddress + ":0" 2> $null
...
function prompt() {
    # Last status
    if ($? -and $LASTEXITCODE.Equals(0)) { Write-Host "$([Char]9829)` " -ForegroundColor Green -NoNewline }
